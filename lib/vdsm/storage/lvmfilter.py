...
def find_lvm_mounts():
    ...
    out = _run(
        [
            ...
            # and "mpath" devices. We will filter the results
            # later to extract only the "lvm" devices.
            "--include",
            dm_major_number(),
...
def vg_info(lv_path):
    ...
    out = _run(
        [
            ...
            # the lvm filter hides the devices
            # of the mounted master lv, and lvs will fail.
            # Use a permissive filter to avoid this.
            # Also, run lvs with devices file disabled.
