    ...
    for lv in _iter_volumes(sdUUID):
        ...
        for tag in lv.tags:
            if tag.startswith(sc.TAG_PREFIX_MD):
                try:
                    offset = int(strip_prefix(tag, sc.TAG_PREFIX_MD))
...
def parse_lv_tags(lv):
    ...
    for tag in lv.tags:
        if tag.startswith(sc.TAG_PREFIX_IMAGE):
            len_prefix_image = len(sc.TAG_PREFIX_IMAGE)
            image = tag[len_prefix_image:]
        elif tag.startswith(sc.TAG_PREFIX_PARENT):
            len_prefix_parent = len(sc.TAG_PREFIX_PARENT)
            parent = tag[len_prefix_parent:]
        elif tag.startswith(sc.TAG_PREFIX_MD):
            try:
                len_prefix_md = len(sc.TAG_PREFIX_MD)
                mdslot = int(tag[len_prefix_md:])
            except ValueError:
                log.warning(
...
class VGTagMetadataRW(object):
    ...
    def readlines(self):
        ...
        metadata = []
        metadata_tag_prefix_len = self.METADATA_TAG_PREFIX_LEN
        for tag in vg.tags:
            if not tag.startswith(self.METADATA_TAG_PREFIX):
                continue

            metadata.append(lvmTagDecode(tag[metadata_tag_prefix_len:]))
...
class BlockStorageDomain(sd.StorageDomain):
        ...
        with open(path, "rb+") as f:
            ...
            with closing(dst):
                for slot in _occupied_metadata_slots(self.sdUUID):
                    ...
                    v4_off_end = v4_off + sc.METADATA_SIZE
                    v4_data = bytes(src[v4_off:v4_off_end])
                    ...
                    v5_off_end = v5_off + sc.METADATA_SIZE
                    dst[v5_off:v5_off_end] = v5_data
    ...
    def _parse_volumes_metadata(self):
        ...
        for slot in slots:
            ...
            offset_with_metadata_size = offset + sc.METADATA_SIZE
            slot_raw_md = raw_md[offset:offset_with_metadata_size]
